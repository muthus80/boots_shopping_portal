...
class Cart(Base):
    ...
    # Response-contract computed fields (CartRead expects these). `items` is
    # eager (lazy="selectin"), so these never trigger a lazy load.
    @property
...
class CartItem(Base):
    ...
    # Response-contract computed fields (CartItemRead expects these). `product`
    # is not eager-loaded by the cart service, so guard against a lazy load in
    # async context — returns empty rather than raising.
    @property
